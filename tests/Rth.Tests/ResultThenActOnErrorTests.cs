...
namespace Rth.Tests
{
    public class ResultThenActOnErrorTests
    {
        [Fact]
        public void PreviousResultsNull()
        {
            ...
            Action thenAct = () => previousResults
                .ThenActOnError<decimal, int, string>(previousOutputs => { })
                .ToList();
        ...
        [Fact]
        public void ThenActOnErrorTests()
        {
            ...
            //!+ Error + OnNotFound
            afterModToLongActualCount.Should().Be(expected: 4 + 1);
            afterLongToDoubleActualCount.Should().Be(expected: 4 + 1);
